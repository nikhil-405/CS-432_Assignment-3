...
class Table:
    ...
    # Export a durable snapshot including the full B+ tree structure.
    def export_snapshot(self):
        return {
            "name": self.name,
            "search_key": self.search_key,
            "tree": self.data.to_dict(),
        }

    # Load a snapshot back into the table; list snapshots are accepted for compatibility.
    def import_snapshot(self, snapshot):
        if isinstance(snapshot, list):
            self.data = BPlusTree(t=max(2, int(self.order)))
            for record in snapshot:
                self.insert(record)
            return

        if not isinstance(snapshot, dict) or "tree" not in snapshot:
            raise ValueError(f"invalid snapshot format for table '{self.name}'")

        snapshot_search_key = snapshot.get("search_key", self.search_key)
        if snapshot_search_key != self.search_key:
            raise ValueError(
                f"snapshot search key '{snapshot_search_key}' does not match table '{self.search_key}'"
            )

        self.data = BPlusTree.from_dict(snapshot["tree"])